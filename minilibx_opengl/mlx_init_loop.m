...
{
  ((mlx_ptr_t *)info)->loop_hook_frames(((mlx_ptr_t *)info)->loop_hook_data_frames);
}
...
void *mlx_init(int *screen_height, int *screen_width)
{
  mlx_ptr_t	*new_mlx;
  int		bidon;
  int		i;
  ...
  new_mlx->loop_hook_frames = NULL;
  new_mlx->loop_hook_data_frames = NULL;
...
int mlx_loop_hook(mlx_ptr_t *mlx_ptr, void (*fct_frames)(void *), void *param_frames)//void (*fct_render)(void *), void *param_render)
{
  CFRunLoopTimerContext	tcontext = {0, mlx_ptr, NULL, NULL, NULL};
  CFRunLoopTimerRef	timer_frames;

  if (mlx_ptr->loop_hook_frames != NULL)
    {
      CFRunLoopTimerInvalidate(mlx_ptr->loop_timer_frames);
      [(id)(mlx_ptr->loop_timer_frames) release];
    }

  mlx_ptr->loop_hook_frames = fct_frames;
  mlx_ptr->loop_hook_data_frames = param_frames;

  if (fct_frames)
    {
      timer_frames = CFRunLoopTimerCreate(kCFAllocatorDefault, 0.0, 0.01, 0, 0, &do_loop_hook2, &tcontext);
      mlx_ptr->loop_timer_frames = timer_frames;
      CFRunLoopAddTimer(CFRunLoopGetMain(), timer_frames, kCFRunLoopCommonModes);
    }
